...
$("#formentri").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url();?>/pinjaman/kriteria/updateutama",
		data:$(this).serialize(),
		error:function(){
...
function showsubkriteria()
{
	$.ajax({
			type:'get',
			dataType:'html',
			url:"<?=base_url('/pinjaman/kriteria/getsubcontainer');?>",
			data:"pinjamanid=<?=$pinjamanid;?>",
			error:function(){
		...
		for($i=1;$i<=$jumlah;$i++)
		{
			...
			if($noSub==$noUtama)
			{
				...
				{									
					...
					for($x=1;$x<=9;$x++)
					{
						$nilai=ambil_nilai_kriteria($pinjamanid,$k2,$xxx);
						$sl='';
						if($nilai==$x)
						{
