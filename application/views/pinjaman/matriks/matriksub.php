...
$("#formentri").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url();?>/pinjaman/kriteria/updatesub",
		data:$(this).serialize(),
		error:function(){
...
$("#prioform").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url();?>/pinjaman/kriteria/updatesubprioritas",
		data:$(this).serialize(),
		error:function(){
...
function rk()
{
	...
	var cii=parseFloat(<?=$jumlah;?>)-1;
	var ci=parseFloat(ci_r_1)/cii;
	var fx_ci=ci;
	$("#sumci").val(fx_ci);
		...
		for($i=1;$i<=$jumlah;$i++)
		{
			...
			if($noSub==$noUtama)
			{
				...
				{									
					...
					for($x=1;$x<=9;$x++)
					{
						$nilai=ambil_nilai_subkriteria($pinjamanid,$kriteriaid,$k2,$xxx);
						$sl='';
						if($nilai==$x)
						{
...
?>
<div class="alert alert-danger">Parameter belum dibuat. Silahkan buat terlebih dahulu <a href="<?=base_url('/master/kriteria/subkriteria?kriteria='.$kriteriaid);?>">Di sini</a> </div>
<?php
}
?>